...
exports.getEnergy = async (req, res, next) => {
  try {
    ...
    await redisClient.set(`energy`, JSON.stringify(energy[0].energy), { expiration: { type: 'EX', value: 5 * 60 } })
...
exports.updateEnergy = async (req, res, next) => {
  ...
  try {
    ...
    if (cachedItems) { // If its already cached
      ...
      await redisClient.set(`energy`, updatedEnergy, { expiration: { type: 'EX', value: 5 * 60 } }) // Updates the cache as well
    ...
    await redisClient.set(`energy`, updatedEnergy, { expiration: { type: 'EX', value: 5 * 60 } }) // Sets the cache for future updates
    await Energy.updateOne({ _id: "688062edebdc5643620fccd6" }, { $set: { energy: updatedEnergy } })
